...
connect() {
	if [ -z "$1" ] || [ -z "$2" ]; then
		...
		to=$($LCLI --lightning-dir="/tmp/l$2-regtest" -F getinfo | grep '^\(id\|binding\[0\]\.\(address\|port\)\)' | cut -d= -f2- | tr '\n' ' ' | (read -r ID ADDR PORT; echo "$ID@${ADDR}:$PORT"))
		$LCLI --lightning-dir="/tmp/l$1-regtest" connect "$to"
	fi
}
